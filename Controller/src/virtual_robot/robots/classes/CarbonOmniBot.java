...
public class CarbonOmniBot extends XDrivePhysicsBase{
    ...
    private final List<Carbon> loadedCarbons = new ArrayList<>();     //List of loaded carbons
    private Carbon carbonToLoad = null;                         //carbon pending loading, if any
    @FXML private Group loadedCarbonGroup;      // Group to represent loaded carbons in the UI
...
//    double rightArmAngleDegrees = 0;
    ...
    // States of the system that pushes loaded carbons into the shooter mechanism
        ...
        switch (kickerState){
            ...
            case SHOOTING:
                if (kickerServo.getInternalPosition() <= 0.2) {
                    ...
                    kickerTimer.reset();
                } else if (kickerServo.getInternalPosition() > 0.8 && kickerTimer.milliseconds() > 50){
...
//        ((Rotate)rightServoArm.getTransforms().get(0)).setAngle(rightArmAngleDegrees);
     ...
     * update.
     *
     * Note that when carbon collision with the intake is detected, the carbon body is not immediately removed from
     * the dyn4j world. During so DURING the world update is not recommended (per dyn4j docs), and does cause
     * problems. Instead, save a reference to the carbon to be loaded, and handle AFTER the world update, within
    ...
    private void shootCarbon(){
        ...
        // Initial position and velocity of the shot carbon (meters and meters/sec)
